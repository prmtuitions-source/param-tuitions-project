...
const VerifyTeachers = () => {
  ...
  const fetchUnverifiedTeachers = async () => {
    try {
      setLoading(true);
      // Fetch profiles where role is 'teacher' and teacher_details.is_verified is NOT true. Also fetch details ID.
  ...
  const handleVerify = async (profileId) => {
    try {
      ...
      // Handle case where teacher_details is array or object to get the correct PK
      ...
      // Remove the verified teacher from the local list
      ...
      alert('Failed to verify teacher. Check console for details.');
    }
  };
