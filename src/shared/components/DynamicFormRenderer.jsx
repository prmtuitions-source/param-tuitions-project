...
const DynamicFormRenderer = ({ formId, onSubmitSuccess, activeSection, submitLabel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    for (const [label, file] of Object.entries(fileUploads)) {
      if (file) {
        try {
          ...
          const filePath = `form-uploads/${user?.id || 'anon'}/${fileName}`;
          ...
          // Save object path; the UI/backend should request a signed URL when needed
          finalFormData[label] = filePath;
        } catch (err) {
          // Error uploading
          uiNotify.alert(`Failed to upload file for ${label}. Please try again.`);
          setSubmitting(false);
          return;
        }
      }
    ...
    if (error) {
      ...
      uiNotify.alert('Request submitted successfully!');
      setFormData({});
      setFileUploads({});
