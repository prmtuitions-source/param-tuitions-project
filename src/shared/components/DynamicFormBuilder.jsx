...
const DynamicFormBuilder = ({ formId, title }) => {
  ...
  const saveForm = async () => {
    ...
    if (error) uiNotify.alert('Error saving form: ' + error.message);
    else uiNotify.alert('Form saved successfully!');
    setLoading(false);
  };
  ...
  const toggleJsonMode = () => {
    if (jsonMode) {
      ...
      uiNotify.alert("Invalid JSON: " + e.message);
      }
    } else {
          ...
          <div key={field.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200 flex flex-col gap-3">
            ...
            {field.type === 'dropdown' && (
              <div>
                ...
                <button 
                  onClick={() => {
                    const pasted = uiNotify.prompt("Paste your list here (one item per line):");
                    if (pasted) {
