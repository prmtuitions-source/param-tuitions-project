    ...
    <div className="mb-4">
      ...
      {value && (
        <div className="mt-2 text-xs text-green-600 font-bold flex items-center gap-1">
          ✓ Uploaded <a href={value} target="_blank" rel="noreferrer" className="underline ml-1">View</a>
        </div>
      )}
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (values.id_proof_type === 'Aadhaar') {
          finalIdProofUrl = values.aadhaar_front_image; 
      ...
      if (activeSection === 'docs' && !isProfileComplete) {
          console.log("Profile Incomplete. Values:", { highestQual, c10: values.class_10_board, addr: values.temporary_address, wa: values.whatsapp_number, photo: values.photo_url, docs: { c10_cert: !!values.class_10_certificate_url, hq_sheet: !!values.highest_qualification_marksheet_url, id_proof: hasValidIdProof, single_aadhaar: !!values.aadhaar_single_file } });
          const missingFields = [];
          ...
          if (!values.highest_qualification_marksheet_url) missingFields.push("Highest Qualification Marksheet");
        ...
        {({ values, setFieldValue, isSubmitting, errors }) => (
          <Form className="space-y-8">
            ...
            <div className="flex gap-4 pt-4">
              ...
              >
                {isSubmitting ? 'Saving...' : (submitLabel || 'Submit Registration')}
